...
app.get('/lends', isAuth, (request, response) => {
	response.send(lends);
});

app.get('/lends/:id', isAuth, (request, response) => {
	const lend = lends.find((l) => l.id === request.params.id);
	response.send(lend);
});

app.post('/lends', isAuth, (request, response) => {
	const isValid = request.body.isbn;
	if (!isValid) {
	...
	lends = [...lends, { ...request.body, id: randomUUID(), borrowedAt: Date.now(), customerId: request.session.email }]
	response.status(201).send(lends);
});

app.delete('/lends/:id', isAuth, (request, response) => {
	lends = lends.map((l) => {
